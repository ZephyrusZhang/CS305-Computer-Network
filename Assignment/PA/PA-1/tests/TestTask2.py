...
class TestTask2(tests.BasicTest.BasicTest):
    ...
    def testIndex(self):
        ...
        resp = requests.head(self.server_base + "data/index.html", headers=self.requests_headers)
        self.assertHTTP200(resp)
    ...
    def test_js(self):
        ...
        resp = requests.head(self.server_base + "data/main.js", headers=self.requests_headers)
        self.assertHTTP200(resp)
        cl = resp.headers.get('Content-Length')
        self.assertTrue(int(cl) == flen)
        self.assertEqual(len(resp.content), 0)
    ...
    def test_sakana(self):
        ...
        resp = requests.head(self.server_base + "data/test.jpg", headers=self.requests_headers)
        self.assertHTTP200(resp)
        cl = resp.headers.get('Content-Length')
        self.assertIn('image/jpeg', str(resp.headers.get('Content-Type')).lower())
        self.assertTrue(int(cl) == flen)
